...
class DynamicalModelTipHycAnnual(DynamicalModel):
    ...
    def derivative(self, states: dict[str, float], forcings: dict[str, float], params: dict[str, float]) -> float:
        ...
        ke_l = self.compute_ke_l(states, forcings, params)
        i_l = self.compute_i(ke_l, forcings)
    ...
    def compute_ke_l(self, states: dict[str, float], forcings: dict[str, float], params: dict[str, float]) -> float:
    ...
    def compute_c_t_from_ke(self, ke, forcings, params):
        ...
        ke_l = ke * params['skc']
        c_t = self.compute_ct_from_ke_l(forcings, ke_l, params)
        return c_t

    def compute_ct_from_ke_l(self, forcings, ke_l, params):
        intermediate_value = (ke_l / params['Ke_max']) ** (-1 / params['b'])
